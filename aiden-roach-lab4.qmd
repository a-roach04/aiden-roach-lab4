...
#| label: load-data
#| eval: FALSE
mbari <- read_table("data-raw/buoy_2024.txt")
```
...
#| eval: FALSE
mbari <- read_table("data-raw/buoy_2024.txt", comment = "#")
```
...
#| eval: FALSE
write_csv(mbari, "data-raw/buoy_update.csv")
```
...
#| eval: FALSE
mbari_clean <- read_csv("data-raw/buoy_update.csv", col_type = list(.default = col_character()), na = c("99", "999"))
```
...
mbari_final <- read_csv("data-clean/buoy_final.csv")

ggplot(mbari_final, aes(x = mm, y = atmp, color = factor(mm))) + 
  geom_point(alpha = 0.2) + 
  geom_jitter(alpha = 0.2, width = 0.20) + 
  geom_smooth(method = NULL, aes(x = mm, y = atmp), color = "black", linetype = "solid") + 
  theme_bw() +
...
https://open.canada.ca/data/dataset/418c81fa-60bd-4063-a526-050c4032ecd1

> This dataset is from Prince Albert National Park in Canada and looks at mammal diversity by using snow tracks in the winter. The dataset was originally published in 2017 but was modified in January of 2025. Some questions that could be answered are how has mammal diversity changed over time, changed with temperature, and/ or changed with the time?
